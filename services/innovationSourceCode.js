/**
* @description REST Api for the InnovationSourceCode team
...
mongo.MongoClient.connect(uristring, function(err, database) {
    if(!err) {
        db = database;
        console.log('InnovationSourceCode Team Connected to the "Hardwarethon_Database" database');
    }
    else{
        console.log(404, 'InnovationSourceCode Team Error Connecting to the "Hardwarethon_Database" database');
    }
});
...
exports.getData = function(req,res) {
    ...
    now.setSeconds(0);
    now.setMilliseconds(0);
    var dateString="logs.get."+now+'.InnovationSourceCode',
        newQuery = {};
    newQuery[dateString]=1;

    db.collection('HardwarethonInfo').findAndModify({},{}, {$inc: newQuery}, {upsert: true, new: true}, function(err, doc_ids){
        db.collection('InnovationSourceCode').find({}, {_id:0}).toArray(function(err, doc){
            if(err) res.send(400, err);
            res.send(200, doc);
        })
    })
